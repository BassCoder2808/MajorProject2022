  ...
  onSubmit = (e) => {
    ...
    if (!this.state.python) {
      ...
      // if code is present, and there is only one comment in the code
        ...
        // post request
        // axios
        //   .post("http://127.0.0.1:9000/codify", { text: comment })
        //   .then((res) => {
        ...
        //   })
        // axios
        //   .get("http://127.0.0.1:8081/parse/conala", {
        //     params: {
        //       q: comment,
        //     },
        //   })
        ...
        //     // console.log("=======");
        //     // console.log(res.data.hypotheses[0]);
        //     let newCodes = res.data.hypotheses.map((item) => [item.value]);
        //     console.log(newCodes);
        //     if (newCodes.length > 0) {
        //       this.setState({ output: newCodes });
        //       // this.setState({ output : [['from sklearn.impute import SimpleImputer', '# define the imputer', "imputer = SimpleImputer(missing_values=nan, strategy='median')", '# transform the dataset', 'transformed_values = imputer.fit_transform(values)', '# count the number of NaN values in each column', "print('Missing: %d' % isnan(transformed_values).sum())", 'from sklearn.impute import SimpleImputer', '# define the imputer', "imputer = SimpleImputer(missing_values=nan, strategy='median')", '# transform the dataset', 'transformed_values = imputer.fit_transform(values)', '# count the number of NaN values in each column', "print('Missing: %d' % isnan(transformed_values).sum())"], ['df.fillna(df.median())', "print('Missing: %d' % isnan(df).sum())"]] });
        //     }
        //   })
        //   .catch((err) => {
        ...
        const url1 = "http://127.0.0.1:8081/parse/conala";
        const url2 = "http://127.0.0.1:9000/codify";
        axios
          .all([
            axios.get(url1, {
              params: {
          ...
          .then(
            axios.spread((res1, res2) => {
              ...
              let newCodes = res1.data.hypotheses.map((item) => [item.value]);
              let bestCodes = res2.data.code;

              const combinedData = [...newCodes, ...bestCodes];

              console.log(newCodes);
              console.log(bestCodes);
              console.log(combinedData);

              if (combinedData.length > 0) {
                this.setState({ output: combinedData });
                // this.setState({ output : [['from sklearn.impute import SimpleImputer', '# define the imputer', "imputer = SimpleImputer(missing_values=nan, strategy='median')", '# transform the dataset', 'transformed_values = imputer.fit_transform(values)', '# count the number of NaN values in each column', "print('Missing: %d' % isnan(transformed_values).sum())", 'from sklearn.impute import SimpleImputer', '# define the imputer', "imputer = SimpleImputer(missing_values=nan, strategy='median')", '# transform the dataset', 'transformed_values = imputer.fit_transform(values)', '# count the number of NaN values in each column', "print('Missing: %d' % isnan(transformed_values).sum())"], ['df.fillna(df.median())', "print('Missing: %d' % isnan(df).sum())"]] });
              }
            })
          )
        ...
        const comment = this.state.python
          .split("#")
          [this.state.python.split("#").length - 1].split("\n")[0];
        console.log(comment);
        // post request
        axios
          .post("http://127.0.0.1:9000/codify", { text: comment })
    ...
    return (
      <div className="App">
        <div className="header">
          ...
          <h5 class="tagline">
            <span>Instantly</span> Convert your <span>Thoughts</span> into{" "}
            <span>Code</span>
          </h5>
          <div class="nav text-center">
            <h5>
              <a href="/Home" className="primary-color text">
                Home
              </a>
            </h5>
